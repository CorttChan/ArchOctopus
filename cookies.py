"""
ArchOctopus 浏览器Cookie模块

参考: "browser-cookie3"第三方库
地址: https://github.com/borisbabic/browser_cookie3

从浏览器cookie数据库中载入指定域名的cookie数据, 参考修改自"browser-cookie3"第三方库.
"""
...
# osx system external dependencies
...
class ChromiumBased:
    ...
    def __add_key_and_cookie_file(self, windows_cookies=None, osx_cookies=None, windows_keys=None,
                                  osx_key_service=None, osx_key_user=None):
...
class Se360(ChromiumBased):
    """Class for 360浏览器(垃圾货色,希望用的人越少越好)"""

    def __init__(self, cookie_file=None, domain_name="", key_file=None):
        args = {
            'windows_cookies': [
                {'env': 'APPDATA', 'path': '360se6\\User Data\\Default\\Cookies'},
            ],
            'windows_keys': [
                {'env': 'APPDATA', 'path': '360se6\\User Data\\Local State'},
            ],
        }

        super().__init__(browser='Se360', cookie_file=cookie_file, domain_name=domain_name, key_file=key_file, **args)


class Sogou(ChromiumBased):
    """Class for 搜狗浏览器(垃圾货色,希望用的人越少越好)"""
    ...
    def __init__(self, cookie_file=None, domain_name="", key_file=None):
        ...
        super().__init__(browser='Sogou', cookie_file=cookie_file, domain_name=domain_name, key_file=key_file, **args)
...
def se360(cookie_file=None, domain_name="", key_file=None):
    """Returns a cookiejar of the cookies used by 360se. Optionally pass in a
    domain name to only load cookies from the specified domain
    """
    return Se360(cookie_file, domain_name, key_file).load()


def sogou(cookie_file=None, domain_name="", key_file=None):
    """Returns a cookiejar of the cookies used by Sogou Explorer. Optionally pass in a
    domain name to only load cookies from the specified domain
    """
    return Sogou(cookie_file, domain_name, key_file).load()
